...
\begin{tabbing}
\\\textbf{8:40:} \hspace{0.5cm} \= Depart campsite
\\\textbf{9:05:} 		\> Stop 1 -- Ice contact lava columns.
...
\\\textbf{12:30:} 		\> Stop 4 --  On top of 2nd Burroughs Mtn.
\\\textbf{13:00:} 		\> Lunch 
\\\textbf{13:30:}		\> Stop 5 -- Pumice of the C tephra
\\\textbf{13:51:}		\> Stop 6 --  Igneous inclusion in andesite
\\\textbf{14:15:}		\> Stop 7 --  Back into granodiorite basement
\\\textbf{15:30:}		\> Stop 8 -- Old mine at Glacial Basin
\\\textbf{16:30:}		\> Hike out to White River exit and return to campsite
\\\textbf{17:30:} 		\> Return to campground
\end{tabbing}
...
\noindent \textbf{11:00 am} -- Starting Hiking \\
We started hiking from the Sunrise parking lot towards Burroughs Mountain, which erupted sometime within the period 480-490 ka. This is the earliest large volume lava flow from the onset of the modern Mt. Rainier edifice.
...
\\\\
3:00 pm -- We came down the Burroughs Mountain Trail to the junction with the Glacial Basin Trail. The team split into two groups. One continued on to the Glacier Basin Camp (1.5 mi round trip), the other headed back directly to the White River Campground parking lot.
...
\begin{tabbing}
\textbf{7:45 am:}  \hspace{0.3cm}\= Depart campsite\\
\textbf{8:53 am:}	\>Arrive at Paradise visitor center\\
\textbf{10:20 am:}	\>Began hike towards Panorama Point\\
\end{tabbing}

\noindent \textbf{7:45 am} -- We left camp without breakfast to beat the Saturday crowds. On the drive around to the south of Ranier we saw plutons from the Tatoosh Range (14 ma) to our south. \\

\noindent \textbf{8:53 am} -- Arrived at Paradise visitor's center to find that the parking lot was already very full, vindicating our early rise. We purchased breakfast at the Paradise Inn (Fig 1) and prepared lunches for the hike.

\noindent \textbf{10:20 am} -- Began hike on trail towards Panorama Point.  Early stages of the trail are paved and crowded. Today we are on the south side of the mountain, yesterday we were on the northeast side.  There are more (and larger) glaciers on the north side of the mountain because it faces away from the sun. After Panorama Point trail, we plan on taking the Glacier Trail. Lenses visible in andesite along trail (Fig 2).  Lenses are lighter colored and contain plagioclase phenocrysts.  Flow banding is visible.

St. Helens visible in the distance to the south (Fig 3).  We can see the north face  of it, including the amphitheater and new dome.  Mt Adams is also visible in the distance.

Phillip's musings:  it's difficult to come up with a universal theory of eruption styles.  The processes are too complex.  My private thoughts:  multi-phase flow is hard, man.  So many different timescales are relevant.  I feel your pain.

We had lunch at Panorama Point (Fig 4).  

Pyroclastics visible on trail up to Glacier Vista, in between andesite flows (I think). Andesite breaks in slaty pieces because of conjugate fracture pattern. The group split up after lunch.  The self-styled “exploration group” (Johnathan, Zach, Jingle, Yang, and myself) climbed two more minor peaks and met up with the main group again at Glacier Vista. At Glacier Vista, Nesqually Glacier is visible in the valley (Fig 5). Little Ice Age scarp and lateral moraine visible on opposite valley wall (Fig 5). The glacier toe is covered in debris (Fig 5). Contact visible in rocks exposed below the LIA scarp on the opposite valley wall.  Speculation about coherent landslide blocks or exposed basement (Fig 6). A waterfall emerging from beneath a small hanging glacier higher on the mountain can be observed entering the subglacial hydrology system beneath Nesqually Glacier (Fig 7).

The deformation structures in the glacier reflect the overall strain rate regime that is expected for a mountain glacier.  Above the equilibrium line altitude (ELA), in the accumulation zone, the glacier experiences extension and thinning to compensate for a positive surface mass balance.  Subsidence caused by extension is (roughly) in balance with snowfall on the surface.  Ice flux increases from the mountain peak towards the ELA, reflecting the increasing volume of snowfall that must be removed by ice flow.  The extension is manifested in brittle failure features visible at the glacier surface, including crevasses and scarps (Figs 8 and 9).  Crevasses are tensile cracks that form when the ice experiences extension or shear (shear stress can be decomposed into compression and tension along the principal axes).  Once they are formed, crevasses are simultaneously advected downstream and deformed by ice flow, resulting in a highly complex pattern of multiple crevasse orientations overprinted on each other in the main glacier trunk (Fig 10).  In addition, crevasses also form to accommodate strain induced by local disturbances in the ice flow, such as turns in the glacier direction and resistant bedrock outcrops below the glacier.  In places, debris-covered portions of the glacier can be distinguished from the surrounding mountainside because crevasses in the underlying ice pull the debris apart (Fig 11).  

Below the ELA, in the ablation zone, the ice experiences compression and thickening to compensate for a negative surface mass balance.  Uplift caused by compression is (roughly) in balance with melting at the surface.  Ice flux decreases from the ELA to the glacier toe, reflecting the increasing volume of ice that has been lost to melting.  Crevasses are less common downstream, reflecting the compressional regime that tends to close them.  Some glaciers have thrust faults in the ablation zone, but these are hard to see from a distance on a debris-covered glacier (they are identified by dipping, arcuate bands of sediment outcropping at the glacier surface) and I don't know if Nesqually has them.  The debris cover on the glacier surface becomes thicker downstream, reflecting the selective removal of ice (but not sediment) by melting.  Thick debris can act to insulate the glacier surface and reduce melting, counteracting the albedo effect seen with thin debris cover.
...
We took the Moraine Trail, which is an “unmaintained” path going out to the crest of one of the LIA lateral moraines of Nesqually Glacier and then back in a loop.  Trail was easy to follow, despite being “unmaintained”.  The return part of the loop included a steep ascent that would have been dangerous going downhill.  Take a right at the fork to make sure you take the steep slope going uphill, which is safer.  Trail runs along the top of the LIA lateral moraine (Fig 13).

Up close we can see that the glacier tongue splits in two, with exposed bedrock in the middle (plutonic Tatoosh?).

The ice surface is extremely dirty, and in places can be hard to distinguish from unvegetated valley floor/walls.  However, the ice surface (where it is exposed) appears darker than the rock and the glacier toe has multiple holes representing abandoned outlets of the subglacial hydrological system (Fig 14).

Both branches of the glacier tongue have water outlets.  The larger branch has the hydrologic outlet roughly in the middle, the smaller branch has an outlet near the contact with the lateral moraine.

Nesqually Glacier is relatively well-studied.  Terminus position has been monitored since the 1940's because of water resource concerns in the downstream river valley.  It advanced between the 1940's and the 1970's, then began a retreat that continues today.  Currently the terminus is in a position similar to where it was in the 1940's.  Retreat can also be inferred between the LIA moraines, deposited in the mid-1800's, and the 1940's, although monitoring was not taking place then.  

Notes on the drive to the next campsite:

Rainier edifice built on Tatoosh granodiorite (Miocene?)

Quartz monzanite.

Welded tuff.  Stretched out clasts.  

Box Canyon is a fracture system in the Tatoosh that is easily erodible.  It has been occupied by both rivers and glaciers.

Welded tuff with andesite dikes.

Welded tuff can have columnar joints.

Ohanapecosh formation, 27-37 ma.  Figure 18 in the USGS guide.

Oligocene intrusive andesites with columnar joints.

Packwood lake.  A landslide-dammed lake (with a small creek for drainage, so it's no longer filling).

Castle Rock. 

Driving through a flat valley.  Flood deposits exposed in river bank.  Floods have good age constrain because of tephra layers.  People live here anyway, extensive farming.

Town of Randall was the base of operations for the IMUSH seismic deployment (which Zach participated in).  
